...
export function addToCart(body) {
  const res = axios.post('https://www.theine.shop/api/cart', body)
          .then (res => res.data)
  return {
    type: ADD_TO_CART,
    payload: res
...
export function getCart(idUser) {
  const res = axios.get('https://www.theine.shop/api/cart', {params: {
    idUser: idUser
  }})
  .then (res => res.data)
  return {
...
export function patchCart(body) {
  const res = axios.patch('https://www.theine.shop/api/cart', body)
          .then (res => res.data)
  return {
    type: PATCH_CART,
    payload: res
...
export function deleteCart(idCart) {
  const res = axios.delete('https://www.theine.shop/api/cart', {params:{
    idCart: idCart,
    idUser: null
  }})
  .then (res => res.data)
